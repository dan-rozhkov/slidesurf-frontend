...
export function PresentationsListSkeleton() {
  return (
...
export default function PresentationsList() {
  const { data: presentations, isLoading } = useQuery({
    ...
    queryFn: presentationsApi.getWithTeamShared,
  });
  const t = useScopedI18n("dashboard");

  if (isLoading || !presentations) {
    return <PresentationsListSkeleton />;
  }
    ...
    <div className="grid grid-cols-4 gap-4 mx-auto max-w-7xl px-4 py-8">
      ...
      {presentations.length === 0 && (
        <div className="col-span-4 flex flex-col gap-4 items-center justify-center w-full min-h-[400px]">
          ...
          <p className="text-sm text-muted-foreground">{t("noPresentations")}</p>
        </div>
      )}
    </div>
  );
